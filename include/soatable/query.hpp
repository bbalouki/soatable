...
/// @note Lazy: it composes view<Cols...>() with std::views::filter. The table must outlive
/// iteration.
...
[[nodiscard]] auto group_sum(
    ...
    return group_reduce<Key, Value>(
        table, key_of, sum_type {}, [value_of](sum_type accumulator, const Value& value) {
...
[[nodiscard]] auto group_count(const basic_soa_table<Storage, Columns...>& table, KeyProj key_of) {
    using key_type = std::decay_t<std::invoke_result_t<KeyProj, const Key&>>;
